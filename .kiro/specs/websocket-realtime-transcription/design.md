...
```mermaid
graph LR
    A[音声バッファ<br/>48kHz stereo<br/>Float32Array] --> B[Whisper large-v3<br/>音声認識<br/>VRAM: 3GB]
...
**クライアント → サーバー**:
```
バイナリフレーム: Float32Array (48kHz, stereo, 32-bit float)
```
...
- **不変条件**: バッファサイズ <= 30秒 * 48kHz * 2ch * 4byte
...
- **TTL**: セッション切断時削除
- **サイズ推定**: 30秒 * 48kHz * 2ch * 4byte = 11.52MB/セッション
...
| エラー | 原因 | 対応 |
|-------|------|------|
| 不正な音声形式 | 48kHz stereo以外のフォーマット送信 | WebSocketサーバーで検証、エラーメッセージ返却 |
...
| T3 | 入力検証 | 音声形式チェック（48kHz, stereo, float32） | Phase 1 |
| T3 | ペイロードサイズ制限 | 最大30秒 * 48kHz * 2ch * 4byte = 11.52MB | Phase 1 |
